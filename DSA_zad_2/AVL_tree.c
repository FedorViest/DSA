...
TREE *AVL_insert(TREE *root, int data){
    ...
    if (root->data == data){        //Ak sa hodnota na vstupe uz nachadza v strome, zvysi sa hodnota pre duplikaty o 1
        root->count++;
        return root;
    }

    else if (data < root->data){        //Pridavanie hodnot do laveho podstromu, ak je hodnota v strome mensia ako na vstupe, funkcia sa vnara dolava
    ...
    return balance_tree(root, data);    //Zavolanie funkcie na vybalansovanie BVS
}
...
TREE *balance_tree(TREE *root, int data){
    ...
    else if (HEIGHT_DIFFERENCE(root) > MIN_DIFFERENCE && data > root->left->data){  //Ak je rozdiel vysok vacsia ako 1 a zaroven hodnota na vstupe je vacsia ako hodnota v strome, vykona sa rotacia vlavo a nasledne rotacia vpravo
        if (root->left != NULL && root->left->right != NULL) {
            return rotate_LeftRight(root);
        }
    }
    else if (HEIGHT_DIFFERENCE(root) < -MIN_DIFFERENCE && data < root->right->data){    //Ak je rozdiel vysok mensia ako -1 a zaroven hodnota na vstupe je mensia ako hodnota v strome, vykona sa rotacia vpravo
        if (root->right != NULL && root->right->left != NULL)
            return rotate_RightLeft(root);
    }
    else if  (HEIGHT_DIFFERENCE(root) < -MIN_DIFFERENCE && data > root->right->data){   //Ak je rozdiel vysok mensia ako -1 a zaroven hodnota na vstupe je vacsia ako hodnota v strome, vykona sa rotacia vpravo a nasledne rotacia vlavo
...
TREE *rotate_left(TREE *root){
    ...
    update_height(root);            //Vyratanie novej vysky pre parent
    update_height(right_child);     //Vyratanie novej vysky pre child
...
TREE *rotate_right(TREE *root){
    ...
    update_height(root);            //Vyratanie novej vysky pre parent
    update_height(left_child);      //Vyratanie novej vysky pre child
...
int update_height(TREE *root){
    return root->height = MAX(GET_HEIGHT(root->right), GET_HEIGHT(root->left)) + 1;
}
...
TREE *AVL_search(TREE *root, int data){
    ...
    if (data < root->data){     //Ak je vstupna hodnota mensia ako hodnota v strome, funkcia sa vnara vlavo
        if(root->left == NULL)
            return NULL;
        else
